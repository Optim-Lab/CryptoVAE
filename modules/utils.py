...
def load_config(config, config_path):
    ...
    return config
#%%
def air_data_generator(df, C, tau):
    n = df.shape[0] - C - tau
...
#%%
def air_data_generator2(df, C, tau):
    n = df.shape[0] - C - tau
    input_data = np.zeros((n, C, df.shape[1]))
...
def visualize_quantile(target_, estQ, colnames, test_len, config, path, show=False, dark=False):
    ...
    return figs
#%%
def air_visualize_quantile(target_, estQ, colnames, config, path, show=False, dark=False):
    mpl.rcParams["figure.dpi"] = 200
    mpl_style(dark=dark)
    SMALL_SIZE = 10
    BIGGER_SIZE = 18
