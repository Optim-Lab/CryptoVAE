...
class Frechet(nn.Module):
    ...
    def quantile_parameter(self, h):
        ...
        theta1 = [h_[:, [0]] for h_ in h] # mu
        theta2 = [nn.Softplus()(h_[:, [1]]) * 0.01 for h_ in h] # sigma
        theta3 = [nn.Softplus()(h_[:, [2]]) * 0.01 for h_ in h] # alpha
    ...
    def quantile_function(self, tau, theta1, theta2, theta3):
        Q = theta1 + theta2 * (-tau.log()) ** (-1/theta3)
        return Q
    ...
    def est_quantile(self, test_context, alphas, MC, disable=False):
        ...
        for a in alphas:
            ...
            for _ in tqdm.tqdm(range(MC), desc=f"Quantile estimation...(alpha={a})", disable=disable):
                ...
                Qs_ = self.quantile_function(alpha, theta1, theta2, theta3)
